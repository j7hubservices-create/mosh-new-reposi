...
const Checkout = () => {
  ...
  // Called when user saves delivery details and clicks "Save & Continue"
  const handleProceedToPayment = (e: React.FormEvent) => {
    e.preventDefault();
    try {
      checkoutSchema.parse(formData);
      toast.success("Delivery details saved. Please select a payment option below.");
      // keep payment panel visible; user must choose payment
  ...
  // Primary payment/order finalization function.
  // Tries client-side Supabase insert first; if RLS or other error occurs,
  // falls back to server endpoint POST /api/create-order (see server code below).
  const handlePayNow = async (e?: React.FormEvent) => {
    ...
    if (selectedPayment === "card") {
      if (!cardData.cardName.trim()) { toast.error("Enter cardholder name"); return; }
      if (cardData.cardNumber.replace(/\s+/g, "").length < 16) { toast.error("Enter a valid 16-digit card number"); return; }
      if (!cardData.expiry.trim() || cardData.expiry.length < 4) { toast.error("Enter valid expiry date (MM/YY)"); return; }
      if (!cardData.cvv.trim() || cardData.cvv.length < 3) { toast.error("Enter valid CVV"); return; }
    }
    ...
    try {
      ...
      if (error) {
        // If error looks like RLS / policy, fall back to server
        throw error;
      }

      // Insert order_items client-side as well
      ...
      toast.success(selectedPayment === "card" ? "Payment successful — order confirmed!" : "Order saved — please complete bank transfer.");
      window.dispatchEvent(new Event("cart-updated"));
      navigate("/thank-you");
    } catch (clientErr: any) {
      // Fallback: call server endpoint. (This is required when RLS prevents client insert.)
      console.warn("Client insert failed, falling back to server endpoint:", clientErr?.message ?? clientErr);
      ...
      try {
        const res = await fetch("/api/create-order", {
          ...
          body: JSON.stringify({
            ...
            // include card info if card payment selected (note: secure handling required on server)
            card: selectedPayment === "card" ? { ...cardData } : undefined,
          }),
        });
        ...
        if (!res.ok) {
          throw new Error(payload?.message || "Server fallback failed");
        }

        // On success, server inserted order and items.
        // Clear cart
        if (user) {
          await supabase.from("cart_items").delete().eq("user_id", user.id);
        } else {
        ...
        toast.error(serverErr?.message || "Failed to process order. Please contact admin.");
      } finally {
        setSubmitting(false);
      }
    } finally {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 py-8 flex-1">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          {/* Right: Order summary + Payment (appears first on mobile) */}
          ...
          <div className="order-2 lg:order-1 lg:col-span-1">
            <Card className="p-6">
              ...
              <form onSubmit={handleProceedToPayment} className="space-y-4">
                ...
                {/* Address or park input (pickup shows store box) */}
