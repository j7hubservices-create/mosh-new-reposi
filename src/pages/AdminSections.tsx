          ...
          <TabsContent value="homepage">
            ...
            <div className="grid gap-4">
              {sections.map(section => (
                <Card key={section.id} className="p-6 flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => { setEditingSection(section); setSectionForm(section); setDialogOpen(true); }}
                    >
          ...
          <TabsContent value="reviews">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <Star className="w-6 h-6 text-primary" /> Customer Reviews
              </h2>
                  ...
                  <form onSubmit={handleReviewSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {[5,4,3,2,1].map(v => <SelectItem key={v} value={v.toString()}>{v} Star{v>1?'s':''}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
            ...
            <div className="grid gap-4">
              {reviews.map(review => (
                <Card key={review.id} className="p-6 flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3 text-sm items-center">
                      ...
                      {review.is_featured && (
                        <span className="px-2 py-0.5 bg-primary/10 text-primary rounded text-xs">Featured</span>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                ...
                <tbody>
                  {orders.map(order => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      <td className="p-2 border font-mono text-sm">{order.id.slice(0, 8)}...</td>
                      ...
                      <td className="p-2 border capitalize">{order.delivery_method}</td>
                      <td className="p-2 border">{new Date(order.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
